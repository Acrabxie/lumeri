...
- R：rename 全量（§0.1 词汇映射表逐行执行，纯机械，语义零变更）。
- K1：遍历内核纯模块 + 单测（未接线）。
- K2：IR v2 切换 + 5 op + 工具 re-plumb + 物化改造 + 测试迁移 + 金标准（原子落地，避免「normalize 已树形而工具仍读扁平」的红灯窗口）。
- C：CLI format.js TOOL_LABELS 追加（quanta 7 工具 + state_scope 分支若可在 format 层承接；App.js 属 execution-intelligence-fix 独占路径，不碰——若分支必须落 App.js，记 QUEUE 交接）。
- V：全量回归对基线快照 + 真机链 draft→assemble→export mp4→抽帧亲验 + pager 目检。
