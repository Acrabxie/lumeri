...
class GemiaOrchestrator:
    ...
    def _resolve_step_var(self, value: Any, context: dict[str, Any]) -> Any:
        """Resolve a variable reference like $input, $output, or $step_N."""
        if isinstance(value, list):
            return [self._resolve_step_var(item, context) for item in value]
        if isinstance(value, dict):
            return {key: self._resolve_step_var(item, context) for key, item in value.items()}
        if value == "$input":
            return context["input_path"]
        if value == "$output":
    ...
    def _resolve_step_output(self, value: Any, step: dict[str, Any], context: dict[str, Any]) -> str:
        """Resolve or allocate a concrete output path for a primitive v2 step."""
        if value == "$output":
            return context["output_path"]
        if isinstance(value, str) and value.startswith("$"):
            step_ref = value[1:]
            if step_ref == step["id"] or step_ref not in context["step_outputs"]:
                return str(self.temp_dir / f"step_{step['id']}_{uuid.uuid4().hex[:6]}.mp4")
            resolved = self._resolve_step_var(value, context)
    ...
    def _step_primitive_v2(self, step: dict[str, Any], context: dict[str, Any]) -> list[str]:
        ...
        if raw_output is not None:
            resolved_output = self._resolve_step_output(raw_output, step, context)
        else:
        ...
        result = PlanEngine(root_dir=self.root_dir)._execute_step(fqn, args, resolved_input, resolved_output)
...
def plan_from_primitives(
    ...
    *,
    ...
    return GemiaOrchestrator().plan_from_primitives(
        request,
        input_path=input_path,
        output_path=output_path,
        answers=answers,
