...
TOOL_SCHEMAS: list[dict[str, Any]] = [
    ...
    _tool(
        ...
        "seed; same brief renders once (content-hash cache).",
        {
            ...
            "feedback": {
                "type": "array",
                "items": {"type": "string"},
                "description": "adjust only: feedback phrases, e.g. ['more playful', 'less dense']. Wording carries DIRECTION only. Sent alone it applies nothing and returns needs:'degree' with the current axes, their meanings, and which way each phrase points; you choose the distance and send `params`. Every create/adjust reply carries `axes`, so normally send feedback + params together in one call."},
            "params": {"type": "object", "description": "adjust: absolute 0..1 axis values YOU chose after reading the wording against the current `axes` (e.g. {\"playfulness\": 0.6}). This is the degree; the engine never guesses it.",
            },
        },
        ["op"],
    ),
    ...
    _tool(
        ...
        "not curve points.",
        {
            ...
            "feedback": {"type": "array", "items": {"type": "string"},
                         "description": "adjust only: phrases like ['warmer','less contrast','更高级']. Wording carries DIRECTION only. Sent alone it applies nothing and returns needs:'degree' with the current axes, their meanings, and which way each phrase points; you choose the distance and send `params`. Every create/adjust reply carries `axes`, so normally send feedback + params together in one call."},
            "params": {"type": "object", "description": "adjust: absolute 0..1 axis values YOU chose after reading the wording against the current `axes` (e.g. {\"warmth\": 0.74}). This is the degree; the engine never guesses it."},
        },
        ["op"],
    ),
    _tool(
        ...
        "op:'catalog' lists layouts/reveals/styles. Deterministic per seed.",
        {
            ...
            "feedback": {"type": "array", "items": {"type": "string"},
                         "description": "adjust only: phrases like ['more energetic','更优雅']. Wording carries DIRECTION only. Sent alone it applies nothing and returns needs:'degree' with the current axes, their meanings, and which way each phrase points; you choose the distance and send `params`. Every create/adjust reply carries `axes`, so normally send feedback + params together in one call."},
            "params": {"type": "object", "description": "adjust: absolute 0..1 axis values YOU chose after reading the wording against the current `axes` (e.g. {\"warmth\": 0.74}). This is the degree; the engine never guesses it."},
        },
        ["op"],
    ),
    _tool(
        ...
        "styles + transition vocabulary.",
        {
            ...
            "feedback": {"type": "array", "items": {"type": "string"},
                         "description": "adjust only: phrases like ['more energetic','fewer transitions']. Wording carries DIRECTION only. Sent alone it applies nothing and returns needs:'degree' with the current axes, their meanings, and which way each phrase points; you choose the distance and send `params`. Every create/adjust reply carries `axes`, so normally send feedback + params together in one call."},
            "params": {"type": "object", "description": "adjust: absolute 0..1 axis values YOU chose after reading the wording against the current `axes` (e.g. {\"warmth\": 0.74}). This is the degree; the engine never guesses it."},
        },
        ["op"],
    ),
    _tool(
        ...
        "op:'adjust' re-derives from feedback; op:'catalog' lists moves. Canvas defaults to the doc.",
        {
            ...
            "feedback": {"type": "array", "items": {"type": "string"},
                         "description": "adjust only: phrases like ['slower','more handheld']. Wording carries DIRECTION only. Sent alone it applies nothing and returns needs:'degree' with the current axes, their meanings, and which way each phrase points; you choose the distance and send `params`. Every create/adjust reply carries `axes`, so normally send feedback + params together in one call."},
            "params": {"type": "object", "description": "adjust: absolute 0..1 axis values YOU chose after reading the wording against the current `axes` (e.g. {\"warmth\": 0.74}). This is the degree; the engine never guesses it."},
        },
        ["op"],
    ),
    _tool(
        ...
        "op:'adjust' re-derives from feedback; op:'catalog' lists framings. Canvas defaults to the doc.",
        {
            ...
            "feedback": {"type": "array", "items": {"type": "string"},
                         "description": "adjust only: phrases like ['tighter','more negative space']. Wording carries DIRECTION only. Sent alone it applies nothing and returns needs:'degree' with the current axes, their meanings, and which way each phrase points; you choose the distance and send `params`. Every create/adjust reply carries `axes`, so normally send feedback + params together in one call."},
            "params": {"type": "object", "description": "adjust: absolute 0..1 axis values YOU chose after reading the wording against the current `axes` (e.g. {\"warmth\": 0.74}). This is the degree; the engine never guesses it."},
        },
        ["op"],
    ),
    _tool(
        ...
        "re-derives from feedback; op:'catalog' lists sync patterns. Deterministic from bpm.",
        {
            ...
            "feedback": {"type": "array", "items": {"type": "string"},
                         "description": "adjust only: phrases like ['tighter','more drive']. Wording carries DIRECTION only. Sent alone it applies nothing and returns needs:'degree' with the current axes, their meanings, and which way each phrase points; you choose the distance and send `params`. Every create/adjust reply carries `axes`, so normally send feedback + params together in one call."},
            "params": {"type": "object", "description": "adjust: absolute 0..1 axis values YOU chose after reading the wording against the current `axes` (e.g. {\"warmth\": 0.74}). This is the degree; the engine never guesses it."},
        },
        ["op"],
    ),
