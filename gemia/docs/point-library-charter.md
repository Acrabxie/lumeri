...
> 版本：**v1.2（RATIFIED · 生效）** · 生效日期：2026-07-15 · 初稿 2026-07-14（4 层心智模型经用户当日批准；v1.1 依五镜对抗评审整改；v1.2 落地 §13.1 五项生效门并转正）
...
> 约定：正文用中文；`code identifiers`、文件路径、token 名（`energy`、`stagger_spread`、`apply_element`、`E_NOT_FOUND` 等）一律保持原形，不翻译。所有路径均在 `/Volumes/Extreme SSD/gemia` 之下（`~/Code/lumeri-design-manuals/` 除外）。
...
- **grading 域（未来 `grading`）**：黑/白点不 clip、skin-tone 落在肤色线、全局饱和 ≤ cap、跨镜色温 ΔK ≤ tolerance、拒绝无脑 teal-orange。
- **kinetic-type 域（未来 `kinetic-type`）**：每行 readability dwell ≥ N（依阅读速度）、measure/行长在带内、在 title-safe 边距内、无 orphan/widow，**外加** motion 地板（因其亦属 TIME 层）。
...
| G15 引用完整性（新增，meta） | §7/§12 引用的每个测试路径都解析到真实 test node | `test_charter_referenced_tests_exist` | true | 【待落地】 | §13 |
...
| **AP33** | **Vaporware gate**（新增，元反模式） | 宪章引用不存在的测试并盖 ✅，把 aspiration 当 enforcement | 引用完整性 meta 门 + DRAFT 生效纪律 `(← §13)` | `test_charter_referenced_tests_exist`【待落地】 |
...
### §11.5 同胞库路线图（route only，尚未建；形态/类别按本次修订重判）
每个新库照 §8 模板建 + 按 §6 装 + 过 §7 全门，并共同触发 §6.3 Layer A/B 门与 §6.6 全局 token 唯一性。**形态判定已依 §2.3（TIME 域必 A）与 §0.7（类别）修正**：

| 未来点库 | 关闭的域 | 类别 | 编码的手册 | 形态判定（修订） |
|---|---|---|---|---|
| `grading` | 调色（static 层） | **TRANSFORM** | 01-grading（preset/LUT/depth） | 若需新 color 原语→A（×TRANSFORM，走 §5T）；若纯组合既有 color-op→B×TRANSFORM |
| `kinetic-type` | 动态排版（**TIME 层**） | SYNTHESIS | 03-subtitle-typography + TIME 层 | **必 Shape A**（TIME 域，§2.3）；走 §5A + type 地板断言 |
| `music-rhythm-edit` | 音乐节奏剪辑（TIME 层） | 混合（读音频+合成剪辑） | 02-editing-rhythm + audiosync | **A**（读音频、新 query 能力，TIME 域） |
| `transition/edit-grammar` | 转场剪辑语法（TIME 层） | SYNTHESIS | 02-editing-rhythm | **A 或 B-内嵌-choreography**（TIME 域，§2.3；不得裸 B 硬编码 easing） |
| `composition/framing` | 构图取景（static 层） | SYNTHESIS/TRANSFORM 视用途 | 04-composition-camera | 倾向 B（静态 preset 走 static 层，P7 不要求 TIME） |
| `camera-movement` | 运镜（**TIME 层**） | SYNTHESIS | 04-composition-camera + TIME 层 | **必 Shape A**（TIME 域，§2.3） |
...
### §13.2 引用完整性守护（meta 门）
`test_charter_referenced_tests_exist`【待落地】扫描本宪章 §7/§12 引用的每个测试标识符，断言其解析到真实 test node；任一为 phantom 即 FAIL。这防止宪章再次"声称拥有不存在的门"（AP33）。
