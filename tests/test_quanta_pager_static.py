...
def test_quanta_pager_is_self_contained_and_referrer_safe() -> None:
    ...
    assert '<link rel="stylesheet" href="quanta.css"' in html
    assert '<script src="quanta.js" defer>' in html
...
def test_main_v3_ui_only_surfaces_same_origin_quanta_pager_urls() -> None:
    ...
    assert "safeQuantaPagerUrl(" in source
    assert "ev.result?.pager_url ?? ev.result?.first_state_pager_url" in source
    assert 'parsed.origin !== window.location.origin' in source
    assert 'parsed.pathname !== "/video/quanta.html"' in source
    assert "打开演示 ↗" in source
