"""search_frames: semantic footage search wired end-to-end through ToolContext.
...
def test_search_frames_is_real_not_stub():
    assert "search_frames" in DISPATCHER
    assert "stub" not in DISPATCHER["search_frames"].__qualname__.lower()
...
def test_finds_and_registers_named_footage(tmp_path):
    ...
    out = _call("search_frames", {"query": "city sunrise", "kind": "video"}, ctx)
    ...
    out2 = _call("search_frames", {"query": "city sunrise", "kind": "video"}, ctx)
...
def test_extra_paths_are_searchable(tmp_path):
    ...
    out = _call("search_frames", {"query": "mountain river", "paths": [str(clip)]}, ctx)
...
def test_empty_when_no_candidates(tmp_path):
    ctx = _ctx(tmp_path)
    out = _call("search_frames", {"query": "anything"}, ctx)
