...
def test_runtime_api_is_feature_flagged(monkeypatch) -> None:
    ...
    response = run_server_handler(
        server._Handler,
        create_raw_request("POST", "/runtime/session", body={"project_id": "proj_rt"}),
    )
...
def test_runtime_session_allows_empty_project_to_reach_sandbox(
    monkeypatch, tmp_path: Path
) -> None:
    monkeypatch.setenv("LUMERAI_VNEXT", "1")
    monkeypatch.setattr(server, "_BASE_DIR", tmp_path)

    session = run_server_handler(
        server._Handler,
        create_raw_request(
            "POST",
            "/runtime/session",
            body={"project_id": "proj_rt", "session_id": "sess_rt"},
        ),
    )
    assert session["status"] == 200
    assert session["body_json"]["status"] == "succeeded"
    assert session["body_json"]["project_id"] == "proj_rt"

    message = run_server_handler(
        server._Handler,
            create_raw_request(
                "POST",
                "/runtime/message?sync=1",
                body={
                    "session_id": "sess_rt",
                    "message": "从空白项目开始做一个视频",
                "script": "raise ValueError('当前运行时还缺少空画布/生成视频 API，不能假装成功')\n",
            },
        ),
    )
    ...
    assert payload["error"]["code"] == "sandbox_failed"
    assert "空画布" in payload["error"]["message"]
    assert "Traceback" not in json.dumps(payload, ensure_ascii=False)

    events = run_server_handler(
        server._Handler,
        create_raw_request("GET", "/runtime/events/sess_rt"),
    )
    assert events["status"] == 200
    assert [event["type"] for event in events["body_json"]["events"]][-1] == "failed"


def test_runtime_message_defaults_to_background_task(monkeypatch) -> None:
    monkeypatch.setenv("LUMERAI_VNEXT", "1")
    ...
    class FakeSessions:
        ...
        def exists(self, session_id):
            return session_id == "sess_async"

        def load_meta(self, session_id):
            return {"project_id": "proj_async"}

        def read_events(self, session_id):
            return list(self.events)

        def append_event(self, session_id, event_type, payload=None):
            event = {"type": event_type, "timestamp": "2026-05-23T00:00:00+00:00", "payload": payload or {}}
    ...
    class FakeRuntimeService:
        ...
        def post_message(self, payload):
            release.wait(3)
            self.sessions.append_event("sess_async", "succeeded", {"task_id": "inner"})
            return {
                "status": "succeeded",
                "session_id": "sess_async",
                "project_id": "proj_async",
                "events": self.sessions.read_events("sess_async"),
            }

    service = FakeRuntimeService()
    monkeypatch.setattr(server, "_runtime_service", lambda: service)

    accepted = run_server_handler(
        server._Handler,
        create_raw_request(
            "POST",
            "/runtime/message",
            body={"session_id": "sess_async", "message": "做一个短视频"},
        ),
    )

    assert accepted["status"] == 202
    assert accepted["body_json"]["status"] == "accepted"
    task_id = accepted["body_json"]["task_id"]
    assert task_id.startswith("rtask_")
    assert any(event["type"] == "runtime_task_started" for event in accepted["body_json"]["events"])

    running = run_server_handler(server._Handler, create_raw_request("GET", f"/runtime/task/{task_id}"))
    assert running["status"] == 200
    assert running["body_json"]["status"] == "running"

    release.set()
    for _ in range(40):
        finished = run_server_handler(server._Handler, create_raw_request("GET", f"/runtime/task/{task_id}"))
        if finished["body_json"]["status"] == "succeeded":
            break
        time.sleep(0.025)

    assert finished["body_json"]["status"] == "succeeded"
    assert finished["body_json"]["result"]["status"] == "succeeded"


def test_runtime_message_sync_escape_hatch(monkeypatch) -> None:
    monkeypatch.setenv("LUMERAI_VNEXT", "1")

    class FakeSessions:
        def exists(self, session_id):
            return session_id == "sess_sync"

        def load_meta(self, session_id):
            return {"project_id": "proj_sync"}
    ...
    class FakeRuntimeService:
        ...
        def post_message(self, payload):
            return {"status": "succeeded", "session_id": payload["session_id"], "project_id": "proj_sync"}

    monkeypatch.setattr(server, "_runtime_service", lambda: FakeRuntimeService())
    ...
    response = run_server_handler(
        ...
        create_raw_request(
            "POST",
            "/runtime/message?sync=1",
            body={"session_id": "sess_sync", "message": "同步执行"},
        ),
    )

    assert response["status"] == 200
    assert response["body_json"]["status"] == "succeeded"
    assert "task_id" not in response["body_json"]


def test_opencode_compat_session_prompt_and_events(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("LUMERAI_VNEXT", "1")
    monkeypatch.setattr(server, "_BASE_DIR", tmp_path)
    ...
    created = run_server_handler(
        ...
        create_raw_request(
            "POST",
            "/session",
            body={"session_id": "oc_sess", "project_id": "oc_proj", "goal": "OpenCode surface"},
        ),
    )

    assert created["status"] == 200
    assert created["body_json"]["id"] == "oc_sess"
    assert created["body_json"]["projectID"] == "oc_proj"
    assert created["body_json"]["version"] == "lumeri-opencode-compat-v0"

    accepted = run_server_handler(
        server._Handler,
        create_raw_request(
            "POST",
            "/session/oc_sess/prompt_async",
            body={"agent": "lumeri-video", "parts": [{"type": "text", "text": "在吗"}]},
        ),
    )
    ...
    messages = None
    for _ in range(40):
        messages = run_server_handler(server._Handler, create_raw_request("GET", "/session/oc_sess/message"))
        text = json.dumps(messages["body_json"], ensure_ascii=False)
        if "在吗" in text and "完成了" in text:
            break
        time.sleep(0.025)

    assert messages is not None
    assert messages["status"] == 200
    assert "在吗" in json.dumps(messages["body_json"], ensure_ascii=False)
    assert any(item["info"]["role"] == "user" for item in messages["body_json"])
    assert any(part["type"] == "text" for item in messages["body_json"] for part in item["parts"])

    events = run_server_handler(server._Handler, create_raw_request("GET", "/event?sessionID=oc_sess"))
    assert events["status"] == 200
    assert events["headers"]["content-type"].startswith("text/event-stream")
    assert b"message.updated" in events["body"]


def test_opencode_compat_shell_and_file_routes(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("LUMERAI_VNEXT", "1")
    monkeypatch.setattr(server, "_BASE_DIR", tmp_path)
    (tmp_path / "hello.txt").write_text("hello opencode\n", encoding="utf-8")
    ...
    run_server_handler(
        ...
        create_raw_request(
            "POST",
            "/session",
            body={"session_id": "oc_shell", "project_id": "oc_shell_proj"},
        ),
    )
    run_server_handler(
        server._Handler,
        create_raw_request("POST", "/runtime/dev/workspace", body={"session_id": "oc_shell"}),
    )
    saved_script = run_server_handler(
        server._Handler,
        create_raw_request(
            "POST",
            "/runtime/dev/workspace/oc_shell/files",
            body={"kind": "scripts", "path": "runtime/script.py", "content": "print('script ok')\n"},
        ),
    )
    assert saved_script["status"] == 200

    shell = run_server_handler(
        server._Handler,
        create_raw_request(
            "POST",
            "/session/oc_shell/shell",
            body={"agent": "lumeri-video", "command": "python3 -c \"print('ok')\""},
        ),
    )

    assert shell["status"] == 200
    assert shell["body_json"]["parts"][0]["type"] == "tool"
    assert shell["body_json"]["parts"][0]["tool"] == "bash"
    assert "ok" in json.dumps(shell["body_json"], ensure_ascii=False)

    script_shell = run_server_handler(
        server._Handler,
        create_raw_request(
            "POST",
            "/session/oc_shell/shell",
            body={
                "agent": "lumeri-video",
                "command": "python3 -c \"from pathlib import Path; Path('previews').mkdir(exist_ok=True); Path('previews/runtime-preview.mp4').write_bytes(b'video')\"",
                "declared_artifact_paths": ["previews/runtime-preview.mp4"],
            },
        ),
    )
    assert script_shell["status"] == 200
    assert script_shell["body_json"]["preview"]["has_preview"] is True
    assert script_shell["body_json"]["preview"]["preview"]["path"] == "previews/runtime-preview.mp4"
    assert script_shell["body_json"]["report"]["brief"]["state"] == "preview_ready"
    assert script_shell["body_json"]["report"]["brief"]["primary_path"] == "previews/runtime-preview.mp4"
    assert script_shell["body_json"]["report"]["summary"]["has_preview"] is True
    assert script_shell["body_json"]["report"]["next_diagnostic"]["kind"] == "review_preview"

    listing = run_server_handler(server._Handler, create_raw_request("GET", "/file?path=."))
    assert listing["status"] == 200
    assert any(item["path"] == "hello.txt" for item in listing["body_json"])

    content = run_server_handler(server._Handler, create_raw_request("GET", "/file/content?path=hello.txt"))
    assert content["status"] == 200
    assert content["body_json"] == {"type": "text", "content": "hello opencode\n"}
