...
CORE_OPS_CATALOG: list[dict[str, Any]] = [
    ...
    {"op": "pip", "group": "transform", "args": ["layer_id*", "corner(br|bl|tr|tl)|x,y", "scale(default 0.3)", "margin(frac, default 0.04)", "radius(px)", "border{color,width}", "shadow(bool)"],
     "summary": "Picture-in-picture: scale the layer down and tuck it into a corner (or explicit x/y) with a rounded-rect mask; optional border/shadow adds one helper shape layer beneath it. Pure transform+mask, no compile change.",
     "example": {"op": "pip", "layer_id": "clip1", "corner": "br", "scale": 0.3, "margin": 0.04, "radius": 24},
     "errors": ["E_ARG when layer_id is missing or corner is unknown", "E_NOT_FOUND when the layer id does not exist", "E_RANGE when scale <= 0 or radius < 0"]},
